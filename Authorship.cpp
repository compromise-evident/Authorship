...
/// Version 3.2, functions: 12,000-digit multi-way. Computational difficulty and super-flexibility
/// in endless cryptographic evidence (publicly-verifiable authorized-only number modification.)
/// Nikolay Valentinovich Repnitskiy - documentation & software license: WTFPLv2+ (wtfpl.net.)

/*Please read the  nine-page  documentation as it is unusually beautiful,  informative,  and graphic  for such a technical project.
You can generate and publish a personal  Authorship number,  preferably before you are subject to  censorship  or worse.  This way,
the public can trust its ownership. Once you become a government threat, the public will inquire about your well being after which,
you modify your number.  Given that only you can modify your number and given that the public or verifying party is here-given the
ability to verify your modification,  we can rest assured that any further correspondence from you is not impersonation.  The best
part is,  you can insert data or any message for every authentication event meaning information source can be trusted. Using these
three properties, Authorship as-is provides the following six functionalities as detailed in the Authorship.pdf documentation file.

* Proof of life    * User authentication    * Canary update authentication    * Data, author, and version control authentication
* Unretentive suspend-able untraceable anonymous cryptocurrency (publicly-verifiable authorized-only withdrawal or ownership hand-over)
* Untraceable anonymous fair trade over any network (cryptographic solution to the Prisoner’s Dilemma)

How to run the program  -  Software package repositories for GNU+Linux operating systems have all the developer tools needed. Open
a terminal and use the following command as root to install Geany and g++ on your computer: apt install geany g++       Geany is a
fast & lightweight text editor and Integrated Development Environment  (IDE)  in which you can write and run code.  g++ is the GNU
compiler for  C++  which allows written code to run.  The compiler operates in the background automatically and displays errors in
your code as you will see in the  lower box in Geany.  Create a folder somewhere on your machine.  Open Geany  and paste this code
into the  blank Geany page.  Now you can get  code-highlighting  and set this document as a  C++  source file.  Go to  Document >>
Set Filetype >> Programming Languages >> C++ source file.  (A good tip for clarity in auditing is to enable the indentation guides
for the block braces.  Go to View >> Show Indentation Guides.)  Save the file as  Authorship.cpp  within the  newly-created folder
(Geany will automatically suggest the  .cpp  extension  since you set the  file  type.)  This folder is now the  home directory of
Authorship.cpp  which will create, write to, and read from  files  within this folder.  And now you can build then run the program.
Use the following Geany shortcuts in order:   F9,   F5.  Whenever it is you wish to use Authorship again, open  Geany  and use the
same shortcuts.  This will work for any  single  C++  file whether you paste over the  current code  or open a new file and repeat.

Without random number generation, functions are produced/verified in seconds.  3  text files you should know about:
-------------------------------------------------------------------------------------------------------------------
Authorship.number   (Storage for another user's  Authorship number,  it will update automatically as they publish.)     1.0 kB
Authorship.public   (Number modification information meant to be published. This file will construct a new number.)  ~138.1 MB
Authorship.private  (*SENSITIVE* Dynamic storage for your Authorship info & keys. This produces Authorship.public.)  ~243.2 MB
-----------------------------------------------------------------------------------------------------------------*/

#include <cstdlib> //For rand() & srand() (user-defined and Unix-time-defined seeds.)
#include <ctime>   //For time function (here only for Unix time.)
#include <fstream> //For file I/O (creates, writes to, and reads from ONLY the three files mentioned above.)
#include <iostream>
using namespace std;

int main()
{	int user_option; //These three functionalities reuse code and run independently (highly modular & audit-friendly.)
	//(You can run all three ifs holding options  1, 2, and 3 in isolation from one another--they are self-sustained.)
	//OPTION 2 ====> line 1020.       OPTION 3 ====>line 2490.
	cout << "\n(Authorship) - infinite authentication tool (version 3.2)\n\n"
	...
	{	//The following block-bunch generates random numbers in array Authorship_private[].
		...
		//(It's important to build sensitive sofware as such on the worst possible machines as this will run on such
		...
		{	//The following block-bunch generates sub-function 1 for the current function. (Cool zone.)
			...
			{	if(Authorship_private[read_bookmark] < 5) {	Authorship_private[read_bookmark]++;} //Reassignes (0=1,1=2,2=3,3=4,4=5.)
				else //Assignes 5 to: 1, 6 to: 2, 7 to: 3, 8 to: 4, and 9 to: 5. (All prime lengths must be 1-5.)
			...
			for(int a = 0; a <= element_SUB2; a++) //Genetates primes of corresponding length & writes them to sub_plaintext_SUB2[].
			...
			for(int a = 0; a <= element_SUB4; a++) //Genetates primes of corresponding length & writes them to sub_plaintext_SUB4[].
			...
			for(int a = 0; a <= element_SUB6; a++) //Genetates primes of corresponding length & writes them to sub_plaintext_SUB6[].
			...
			}                             //write_bookmark evenually catches up. This block forces read_bookmark to jump dynamically.
		...
		//Constructively combines the comression tables. compression_119[] will hold it all.
		for(int a = 0; a < 100; a++)
		...
		for(int a = 0; a < 256500000; a++)
		{	Authorship_private[a] = 9;
			Authorship_private[a] = 8;
			Authorship_private[a] = 7;
			Authorship_private[a] = 6;
			Authorship_private[a] = 5;
			Authorship_private[a] = 4;
			Authorship_private[a] = 3;
			Authorship_private[a] = 2;
			Authorship_private[a] = 1;
			Authorship_private[a] = 0;
		}
		...
		cout << "\n\n\nFinished.\n\n\n"
			 ...
			 << "Your number resides in the file Authorship.number in this directory, publish it!\n"
			 << "Your keys reside in Authorship.private in this directory, cache them guardedly.\n\n\n\n";
	}
	...
	if(user_option == 2)
	{	//BASIC LAYOUT:
		//1. Generates your new number & keys (same as Authorship option 1.)
		//2. Loads given old Authorship.private file into old[].
		//3. Overwrites given Authorship.private flie with new keys from step 1.
		...
		//Loads old Authorship.private into old[].
		...
		//# 77		1 0 1 0 1 0 1 0 1		76			l	(l as in linear)   (108)
		...
		//# 96		1 1 0 0 1 0 0 1 1		95			no char*
		...
		//(Combination 96 is a custom assignment in the Authorship program. It is NOT the null character.)
		
		
		
		
		
		//Prints the new number incase the user wants to see it, publish it, or send it for trade. Numbers are normally
		//handled automatically. This option to see the new number allows any newcommer to begin verifying authentication
		...
		for(int a = 0; a < 500; a++) //Loads binary expansion for number into b[]. (The first 500 groups of nine in b[] will represent the Authorship number, each group giving a digit pair.)
		{
			...
			else {cout << "\n\nCosmic ray corruption, line 2216.\n\n"; return 0;}
		}
		...
		//Message or not, remaining b[] elements are filled with reference # 96 (null = 1 1 0 0 1 0 0 1 1.)
		...
		cout << "Enter a message (1,000 characters max, will be truncated) otherwise press enter:\n\n";
		char nothing_to_see_here[1];
		cin.getline(nothing_to_see_here, 1); //Catching cache.
		
		char message[10000]; //This is TEN thousand incase the user enters too many.
		...
		{	m = 4500; //Again, sets write head of b[] to where the user message will begin in "5/9 binary." b[4500] is the 4,501st element.
			...
			{	
				//                  message
				//                 character      |           |           |           |           |           |           |           |           |
					 if(message[a] == ' ') {b[m]= 0;  b[m+1]= 0;  b[m+2]= 0;  b[m+3]= 0;  b[m+4]= 1;  b[m+5]= 1;  b[m+6]= 1;  b[m+7]= 1;  b[m+8]= 1;  m+=9;} //(Blank or space.)
				...
				else {cout << "\n\nCosmic ray corruption, line 2343.\n\n"; return 0;}
			}
		}
		...
		//Selectively writes old[] to Authorship.public BASED ON THE CONTENTS OF b[]. (Authorship.public is a new file created now.)
		bool hash_digits_table[12006] = {0}; //Prepares a hash-digits distribution table who will dictate which items to omit and which to write to file Authorship.public.
		...
		{	if(b[revelation] == 0)
			{	/// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				{	if(hash_digits_table[a] == 0) {old_read_bookmark++;}
					...
					{	if(old[old_read_bookmark] == 48) {out_stream << '0'; old_read_bookmark++; continue;}
						if(old[old_read_bookmark] == 49) {out_stream << '1'; old_read_bookmark++; continue;}
						if(old[old_read_bookmark] == 50) {out_stream << '2'; old_read_bookmark++; continue;}
						if(old[old_read_bookmark] == 51) {out_stream << '3'; old_read_bookmark++; continue;}
						if(old[old_read_bookmark] == 52) {out_stream << '4'; old_read_bookmark++; continue;}
						if(old[old_read_bookmark] == 53) {out_stream << '5'; old_read_bookmark++; continue;}
						if(old[old_read_bookmark] == 54) {out_stream << '6'; old_read_bookmark++; continue;}
						if(old[old_read_bookmark] == 55) {out_stream << '7'; old_read_bookmark++; continue;}
						if(old[old_read_bookmark] == 56) {out_stream << '8'; old_read_bookmark++; continue;}
						if(old[old_read_bookmark] == 57) {out_stream << '9'; old_read_bookmark++; continue;}
					}
				}
			...
			{	/// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				{	if(old[old_read_bookmark] == 48) {out_stream << '0'; old_read_bookmark++; continue;}
					...
					if(old[old_read_bookmark] == 56) {out_stream << '8'; old_read_bookmark++; continue;}
					if(old[old_read_bookmark] == 57) {out_stream << '9'; old_read_bookmark++; continue;}
					if(old[old_read_bookmark] == 46) {out_stream << '.'; old_read_bookmark++; continue;}
				}
			}
		...
		cout << "\n\n\n\n\n\n\n\n\n\n\nFinished.\n\n\n"
			 ...
			 << "Cache them guardedly and destroy the old copy file.\n\n"
	...
	//________________________________________________________________________________________________________________________
	//                                                                                                                       |
	//                                         3   Verifiy number modification                                               |
	//_______________________________________________________________________________________________________________________|
	if(user_option == 3)
	{	//Basic layout:
		//1. Reads file Authirship.public then generates the compression.
		...
		cout << "\n>>>>>>>>>>>>>>>>>>>>>>>>>>(Verify number modification)<<<<<<<<<<<<<<<<<<<<<<<<<<\n"
			 << "Place COPIES of the following two files in this directory if not already here.\n\n"
		
			 << "    Authorship.number  (User's number as saved on your device.)\n"
		...
		//Loads Authorship.public into Authorship_public[].
		...
		{		 if(Authorship_public[a] == 48) {Authorship_public[a] = 0;}
			...
			else {cout << "\n\nCosmic ray corruption, line 2540.\n\n"; return 0;}
		}
		...
		//The following block-bunch generates the compression and compares it to Authorship.number.
		//The following guardians must ALL sing "true" before the verifying party gets the message "Verification SUCCESSFUL!"
		//These guardians exist for exceptional clarity and safety in the many diverse devices who may run the Authorship program.
		//Any errors or mismatch in the verification process IMMEDIATELY ends the program with the message "Verification FAILED!"
		//This ensures that false authentication events are terminated here long before any chance of sliding under additional guardians.
		//Guardians may be controlled by external means at any moment, by accident in overflow or with clever attacks.
		//Guardians sing "false" by default. (Sensitive software means safety first!)
		...
		bool characters = false; //The remainig 1,000 groups of nine contiguous functions must correspond to character assignment.
		...
		//even halt the download early as you can process dowloaded data sequentially. Surprisingly, this is not a solution still. And given the
		...
		//in the first considerstion. This helps pin-point which number the file inquires to modify, however, the remaining file content could be faulty
		...
		{	in_stream >> Authorship_number[a];
			...
			else {cout << "\n\nCosmic ray file corruption, line 2840.\n\n"; return 0;}
		}
		...
		for(int a = 0; a < 1000; a++)
		{	if(compression[a] != Authorship_number[a]) {cout << "\n\n\nVerification FAILED! Numbers mismatch! 2848\n\n\n"; return 0;}
		...
		{	/// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			{	b[f] = 1;
				...
				for(int a = 0; a < 2000; a++) {read_bookmark_crypt++;} //Skips through 1st sub-ciphertext.
				read_bookmark_crypt++;
				...
				for(int a = 0; a < 2000; a++) {read_bookmark_crypt++;} //Skips through 3rd sub-ciphertext.
				read_bookmark_crypt++;
				...
				for(int a = 0; a < 2000; a++) {read_bookmark_crypt++;} //Skips through 5th sub-ciphertext.
				read_bookmark_crypt++;
			...
			//reproduced exactly and as expected. This leaves quite the space
			//for those who need to stuff as much data into these files as possible.
		...
		{	
			...
			{	cout << "There is no message for this authentication event.\n";
			}
		}
