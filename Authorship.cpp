...
/* Version 4.5, 12k-digit multi-way functions. This is computational difficulty
...
Authorship.public   (Number modification information meant to be       ~168.2 MB
                    published. This file will construct a new number.)
Authorship.private  (*SENSITIVE* Dynamic storage for your Authorship   ~237.9 MB
...
{	int user_option;
	...
	{	ifstream in_stream;
		...
		cout << "\n>>>>>>>>>>>>>>>>>>>>>>>(Get your first Authorship number)<<<<<<<<<<<<<<<<<<<<<<<\n"
		     << "Enter 8 random digits, repeat 50 times. (Get 238MB worth of keys in 20 minutes.)\n\n";
		...
		cout << "\n\n\n\n\nGenerating randomness...\n"; //TIP: printing amid heavy computation requires printing new lines so it prints.
		...
		long long int deductive_lossy_compression[100]; //Essentially, this is 100 different hash functions.
		long long int                    snapshot[100]; //Takes a snapshot of deductive_lossy_compression[] every once in a while.
		
		for(int a = 0; a < 100; a++) {deductive_lossy_compression[a] = 5555555555; snapshot[a] = 5555555555;}
		...
		{	//The following block-bunch generates sub-function 1 for the current function. (Cool zone.)
			...
			int prime_lengths_in_order_SUB2[570]; //Expected contiguous primes: ~535. Tolerance at verification: 500 - 570.
			...
			{	if(Authorship_private[read_bookmark] < 5) {Authorship_private[read_bookmark]++;} //Reassigns (0=1,1=2,2=3,3=4,4=5.)
				else                                   {Authorship_private[read_bookmark] -= 4;} //Reassigns (5=1,6=2,7=3,8=4,9=5.) (All prime lengths are 1-5.)
				
				//Reducing the chances of generating 1 and 2-digit primes.
				if(Authorship_private[read_bookmark] < 3)
				{	int one_in_ten = (rand() % 10);
					if(one_in_ten != 0) {Authorship_private[read_bookmark] += 2;}
				}
			...
			{	if(prime_lengths_in_order_SUB2[a] == 1) //Randomly generates a single-digit prime.
				...
				{	int temp_5_digit_prime;
					...
					{	if(temp_5_digit_prime > 99991) {temp_5_digit_prime = 99991; break;} //If exceeds length of 5, sets it to largest 5-digit prime.
						temp_5_digit_prime++;
					}
			...
			int prime_lengths_in_order_SUB4[570]; //Expected contiguous primes: ~535. Tolerance at verification: 500 - 570.
			...
			int prime_lengths_in_order_SUB6[570]; //Expected contiguous primes: ~535. Tolerance at verification: 500 - 570.
			...
			int temp_write_bookmark = (write_bookmark - 2);
			for(int a = 0; a < 100; a++)
			{	for(int b = 0; b < 2000; b++)
				{	deductive_lossy_compression[a] += Authorship_private[temp_write_bookmark];
					deductive_lossy_compression[a] *= (a + 1); //Each of the 100 hash functions have a unique property of multiplication.
					deductive_lossy_compression[a] %= 10000000000;
					temp_write_bookmark --;
				}
				
				temp_write_bookmark = (write_bookmark - 2); //Resetting.
			}
			
			temp_write_bookmark = (write_bookmark - 4004);
			for(int a = 0; a < 100; a++)
			{	for(int b = 0; b < 2000; b++)
				{	deductive_lossy_compression[a] += Authorship_private[temp_write_bookmark];
					deductive_lossy_compression[a] *= (a + 1); //Each of the 100 hash functions have a unique property of multiplication.
				...
				temp_write_bookmark = (write_bookmark - 4004); //Resetting.
			}
			
			temp_write_bookmark = (write_bookmark - 8006);
			for(int a = 0; a < 100; a++)
			{	for(int b = 0; b < 2000; b++)
				{	deductive_lossy_compression[a] += Authorship_private[temp_write_bookmark];
					deductive_lossy_compression[a] *= (a + 1); //Each of the 100 hash functions have a unique property of multiplication.
				...
				temp_write_bookmark = (write_bookmark - 8006); //Resetting.
			}
			...
			{	for(int a = 0; a < 100; a++)
				{	snapshot[a] += deductive_lossy_compression[a];
					snapshot[a] %= 10000000000; //(10^10, results in last ten digits shown.)
				}
			}
		...
		for(int a = 0; a < 100; a++)
		{	deductive_lossy_compression[a] += snapshot[a];
			deductive_lossy_compression[a] %= 10000000000; //(10^10, results in last ten digits shown.)
		}
		deductive_lossy_compression[ 0] += snapshot[33]; deductive_lossy_compression[ 0] %= 10000000000;
		deductive_lossy_compression[99] += snapshot[66]; deductive_lossy_compression[99] %= 10000000000;
		
		for(int a = 0; a < 100; a++) //Ensures each constituent compression integer is 10 digits long. The Authorship number is now complete.
		{	if(deductive_lossy_compression[a]   < 1000000000)
			{	deductive_lossy_compression[a] += 1000000000;
			}
		}
		...
		for(int a = 0; a < 100; a++)
		{	out_stream << deductive_lossy_compression[a];
		}
		out_stream.close();
		...
		for(int a = 0; a < 100; a++)
		{	compression[compression_write_bookmark] =  (deductive_lossy_compression[a] / 1000000000);
			compression_write_bookmark++;
			compression[compression_write_bookmark] = ((deductive_lossy_compression[a] / 100000000) % 10);
			compression_write_bookmark++;
			compression[compression_write_bookmark] = ((deductive_lossy_compression[a] / 10000000) % 10);
			compression_write_bookmark++;
			compression[compression_write_bookmark] = ((deductive_lossy_compression[a] / 1000000) % 10);
			compression_write_bookmark++;
			compression[compression_write_bookmark] = ((deductive_lossy_compression[a] / 100000) % 10);
			compression_write_bookmark++;
			compression[compression_write_bookmark] = ((deductive_lossy_compression[a] / 10000) % 10);
			compression_write_bookmark++;
			compression[compression_write_bookmark] = ((deductive_lossy_compression[a] / 1000) % 10);
			compression_write_bookmark++;
			compression[compression_write_bookmark] = ((deductive_lossy_compression[a] / 100) % 10);
			compression_write_bookmark++;
			compression[compression_write_bookmark] = ((deductive_lossy_compression[a] / 10) % 10);
			compression_write_bookmark++;
			compression[compression_write_bookmark] =  (deductive_lossy_compression[a]      % 10);
			compression_write_bookmark++;
		}
		...
		cout << "\n\n\nFinished.\n\n\n"
		     ...
		     << "Please include the Authorship version used, and your name, nickname, or any tag.\n"
	...
	{	ifstream in_stream;
		...
		//# 77      1 0 1 0 1 0 1 0 1       76          l   (l as in lambent)  (108)
		...
		cout << "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n";
		for(int a = 0; a < 100; a++) {cout << deductive_lossy_compression[a];}
		...
		{	pair[pair_write_bookmark    ] = deductive_lossy_compression[a] % 100; deductive_lossy_compression[a] /= 100;
			pair[pair_write_bookmark - 1] = deductive_lossy_compression[a] % 100; deductive_lossy_compression[a] /= 100;
			pair[pair_write_bookmark - 2] = deductive_lossy_compression[a] % 100; deductive_lossy_compression[a] /= 100;
			pair[pair_write_bookmark - 3] = deductive_lossy_compression[a] % 100; deductive_lossy_compression[a] /= 100;
			pair[pair_write_bookmark - 4] = deductive_lossy_compression[a];
		...
		{	m = 4500; //Again, sets write head of b[] to where the user message will begin in "5/9 binary." b[4500] is the 4,501st element.
			...
			{	
				...
				else {cout << "\n\nCosmic ray corruption. Did you hit an arrow key or backspace?\n\n"; return 0;}
			}
		}
		...
		//Prepares a table that dictates which function digits to omit and which to write to file Authorship.public.
		bool hash_digits_table[12006] = {0};
		for(int marker =  12004, a = 0; a < 2000; a++) {hash_digits_table[marker] = 1; marker--;}
		for(int marker =   8002, a = 0; a < 2000; a++) {hash_digits_table[marker] = 1; marker--;}
		for(int marker =   4000, a = 0; a < 2000; a++) {hash_digits_table[marker] = 1; marker--;}
		...
		{	if(b[revelation] == 0)
			{	/// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				for(int a = 0; a < 12006; a++) ///Writes only the hash-digits of the function (6,000 digits then one separating dot.) //////////////////////
		...
		cout << "\n\n\n\n\n\n\n\n\n\n\nFinished.\n\n\n"
		     ...
		     << "Cache them guardedly and destroy the old copy file. You don't\n"
		     << "need YOUR Authorship.number file, numbers will be intertwined\n"
		     << "in your Authorship.public files, as well as being printed in the\n"
	...
	{	ifstream in_stream;
		...
		static char Authorship_public[170000000];
		in_stream.open("Authorship.public");
		for(int a = 0; a < 170000000; a++)
		...
		//to read and verify the remaining 168.19999MB. But let's get clever. Suppose the first string read from the file contains a cryptographic key
		...
		//Prepares a table that dictates which function digits to omit and which to utilize for generating the hash.
		bool hash_digits_table[12006] = {0};
		for(int marker =  12004, a = 0; a < 2000; a++) {hash_digits_table[marker] = 1; marker--;}
		for(int marker =   8002, a = 0; a < 2000; a++) {hash_digits_table[marker] = 1; marker--;}
		for(int marker =   4000, a = 0; a < 2000; a++) {hash_digits_table[marker] = 1; marker--;}
		...
		{	if(Authorship_public[read_bookmark + 6000] == '.') //If chunk of hash-digits, simulates them in the length of a real function.
			{	int simulated_function[12006];
				...
				for(int a = 0; a < 100; a++)
				{	for(int b = 0; b < 2000; b++)
					{	deductive_lossy_compression[a] += simulated_function[temp_read_bookmark];
						deductive_lossy_compression[a] *= (a + 1); //Each of the 100 hash functions have a unique property of multiplication.
						deductive_lossy_compression[a] %= 10000000000;
						temp_read_bookmark--;
					}
					
					temp_read_bookmark = 12004; //Resetting.
				}
				
				temp_read_bookmark = 8002;
				for(int a = 0; a < 100; a++)
				{	for(int b = 0; b < 2000; b++)
					{	deductive_lossy_compression[a] += simulated_function[temp_read_bookmark];
						deductive_lossy_compression[a] *= (a + 1); //Each of the 100 hash functions have a unique property of multiplication.
					...
					temp_read_bookmark = 8002; //Resetting.
				}
				
				temp_read_bookmark = 4000;
				for(int a = 0; a < 100; a++)
				{	for(int b = 0; b < 2000; b++)
					{	deductive_lossy_compression[a] += simulated_function[temp_read_bookmark];
						deductive_lossy_compression[a] *= (a + 1); //Each of the 100 hash functions have a unique property of multiplication.
					...
					temp_read_bookmark = 4000; //Resetting.
				}
			...
			{	read_bookmark += 12006;
				int temp_read_bookmark = (read_bookmark - 2);
				for(int a = 0; a < 100; a++)
				{	for(int b = 0; b < 2000; b++)
					{	deductive_lossy_compression[a] += Authorship_public[temp_read_bookmark];
						deductive_lossy_compression[a] *= (a + 1); //Each of the 100 hash functions have a unique property of multiplication.
						deductive_lossy_compression[a] %= 10000000000;
						temp_read_bookmark--;
					}
					
					temp_read_bookmark = (read_bookmark - 2); //Resetting.
				}
				
				temp_read_bookmark = (read_bookmark - 4004);
				for(int a = 0; a < 100; a++)
				{	for(int b = 0; b < 2000; b++)
					{	deductive_lossy_compression[a] += Authorship_public[temp_read_bookmark];
						deductive_lossy_compression[a] *= (a + 1); //Each of the 100 hash functions have a unique property of multiplication.
					...
					temp_read_bookmark = (read_bookmark - 4004); //Resetting.
				}
				
				temp_read_bookmark = (read_bookmark - 8006);
				for(int a = 0; a < 100; a++)
				{	for(int b = 0; b < 2000; b++)
					{	deductive_lossy_compression[a] += Authorship_public[temp_read_bookmark];
						deductive_lossy_compression[a] *= (a + 1); //Each of the 100 hash functions have a unique property of multiplication.
					...
					temp_read_bookmark = (read_bookmark - 8006); //Resetting.
				}
			}
		...
		{	/// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if(Authorship_public[read_bookmark_crypt + 6001] == 'x')
			{	read_bookmark_crypt += 6003;
			...
			{	b[f] = 1;
				...
				//Checking for tolerated quantities of primes per hot zone.
				int tolerance_counter_SUB2 = 0;
				...
				{	prime_lengths_in_order_SUB2[a] = Authorship_public[read_bookmark_crypt];
					read_bookmark_crypt++;
					tolerance_counter_SUB2++;
				}
				read_bookmark_crypt++;
				
				int tolerance_counter_SUB4 = 0;
				...
				{	prime_lengths_in_order_SUB4[a] = Authorship_public[read_bookmark_crypt];
					read_bookmark_crypt++;
					tolerance_counter_SUB4++;
				}
				read_bookmark_crypt++;
				
				int tolerance_counter_SUB6 = 0;
				...
				{	prime_lengths_in_order_SUB6[a] = Authorship_public[read_bookmark_crypt];
					read_bookmark_crypt++;
					tolerance_counter_SUB6++;
				}
				read_bookmark_crypt++;
				
				if((tolerance_counter_SUB2 < 500)
				|| (tolerance_counter_SUB4 < 500)
				|| (tolerance_counter_SUB6 < 500)
				
				|| (tolerance_counter_SUB2 > 570)
				|| (tolerance_counter_SUB4 > 570)
				|| (tolerance_counter_SUB6 > 570))
				{cout << "\n\n\nVerification FAILED!     Bad randomness/Attemped attack.\n\n\n"; return 0;}
